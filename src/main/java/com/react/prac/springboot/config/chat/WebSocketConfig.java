package com.react.prac.springboot.config.chat;
...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    private final StompInterceptorHandler stompHandler;

    // STOMP 엔드포인트 등록
    @Override
    public void registerStompEndpoints(StompEndpointRegistry registry) {
        registry.addEndpoint("/ws-stomp")  // STOMP 엔드포인트 설정
//                .setAllowedOrigins("http://localhost:3000") // 모든 Origin 허용 -> 배포시에는 보안을 위해 Origin을 정확히 지정
//                .setAllowedOrigins("http://localhost:8080")
                .setAllowedOrigins(ChatUtil.redirectUrl)
